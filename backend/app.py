...
def expiry_date():
    ...
    cursor.execute("""
        ...
        WHERE expiry_date BETWEEN CURDATE() AND DATE_ADD(CURDATE(), INTERVAL 30 DAY)
    """)
    soon = cursor.fetchall()

    return render_template("expiry_date.html", expired=expired, soon=soon)
...
def analytics():
    ...
    cursor.execute("""
        SELECT drug_name, drug_price
        FROM medicines
        ORDER BY drug_price ASC
        LIMIT 1
    """)
    lowest_price = cursor.fetchone()

    cursor.execute("""
        SELECT AVG(drug_price)
        FROM medicines
    """)
